...
class CreateOneappRewardEventsRuleTable extends Migration {
	...
	public function up()
	{
		Schema::create('oneapp_reward_events_rule', function(Blueprint $table)
		{
			$table->increments('re_id')->comment('系統流水號');
			$table->bigInteger('rem_id')->nullable()->default(0)->comment('refer to oneapp_reward_events_main.rem_id');
			...
			$table->boolean('get_points_first_flag')->nullable()->default(0)->index('idx_get_points_first_flag')->comment('是否先贈點 (0=兌換券全收核銷後給點，2=單張兌換券核銷後給點) [禁用(程式會忽略)]1=交易成單給點');
	...
	{
		Schema::drop('oneapp_reward_events_rule');
	}
