...
class CreateBuy123EdmInfoTable extends Migration {
	...
	public function up()
	{
		Schema::create('buy123_edm_info', function(Blueprint $table)
		{
			...
			$table->dateTime('edm_date')->default('0000-00-00 00:00:00')->index('idx_edm_date')->comment('edm 發送日期');
			$table->dateTime('create_dt')->default('0000-00-00 00:00:00')->index('idx_create_dt')->comment('建立時間');
			$table->string('create_user', 30)->default('')->comment('建立人員');
			$table->dateTime('modify_dt')->default('0000-00-00 00:00:00')->index('idx_modify_dt')->comment('最後修改時間');
