...
class CreateTravelcardPostbackTable extends Migration {
	...
	public function up()
	{
		Schema::connection('transaction')->create('travelcard_postback', function(Blueprint $table)
		{
			...
			$table->dateTime('sent_ts')->nullable();
			$table->dateTime('sent_ts')
			->default('0000-00-00 00:00:00')->index('idx_sent_ts')->comment('電文發送時間')->change();
			$table->dateTime('update_ts')->nullable();
			$table->dateTime('update_ts')
			->default('0000-00-00 00:00:00')->comment('電文更新時間')->change();
		});
	}
