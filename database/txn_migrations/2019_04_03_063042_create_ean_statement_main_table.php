...
class CreateEanStatementMainTable extends Migration {
	...
	public function up()
	{
		Schema::connection('transaction')->create('ean_statement_main', function(Blueprint $table)
		{
			...
			$table->dateTime('ean_checkin_dt')->nullable();
			$table->dateTime('ean_checkin_dt')
			->default('0000-00-00 00:00:00')->comment('ean記錄的入住時間')->change();
			$table->dateTime('ean_checkout_dt')->nullable();
			$table->dateTime('ean_checkout_dt')
			->default('0000-00-00 00:00:00')->comment('ean記錄的退房時間')->change();
			...
			->default('0000-00-00 00:00:00')->comment('建立時間')->change();
