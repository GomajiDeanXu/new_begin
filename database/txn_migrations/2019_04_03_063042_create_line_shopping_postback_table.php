...
class CreateLineShoppingPostbackTable extends Migration {
	...
	public function up()
	{
		Schema::connection('transaction')->create('line_shopping_postback', function(Blueprint $table)
		{
			...
			$table->dateTime('order_dt')->default('0000-00-00 00:00:00')->index('idx_order_dt')->comment('付費時間, 對應 transaction.user_purchases.paid_ts');
			...
			$table->dateTime('feetime')->default('0000-00-00 00:00:00')->index('idx_feetime')->comment('銷售認列時間(所有 coupon 都需核銷並抓最後一個序號核銷的時間)');
			$table->dateTime('feetimetoline')->default('0000-00-00 00:00:00')->comment('對帳認列時間(拋給Line)');
			$table->string('ecid')->default('')->index('idx_ecid')->comment('line用戶識別碼');
			$table->dateTime('first_postback_dt')->default('0000-00-00 00:00:00')->index('idx_first_postback_dt')->comment('首次回拋時間');
			...
			$table->dateTime('deposit_postback_dt')->default('0000-00-00 00:00:00')->index('idx_deposit_postback_dt')->comment('請款回拋時間');
